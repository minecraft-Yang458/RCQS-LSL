> 本仓库作者已与原作者于2026年8月30日达成合作协议，本仓库已转正为Godot重构版。

<img src="https://github.com/minecraft-Yang458/RCQS-LSL/blob/main/project/assets/36A2E601C6DEECC501B07DA113489E13.png" alt="logo" height="200px" width="300px">
# 《日常侵蚀 灵蚀楼》Godot重构版
![Godot Engine](https://img.shields.io/badge/Godot-4.7.2-478CBF?logo=godotengine&logoColor=white)
![GDScript](https://img.shields.io/badge/GDScript-语言-478CBF)
![平台](https://img.shields.io/badge/平台-Windows%20%7C%20Linux%20%7C%20macOS%20%7C%20Android%20%7C%20iOS-blue)<br>
[![代码许可证: GPLv3](https://img.shields.io/badge/代码许可证-GPLv3-green.svg?style=flat)](https://www.gnu.org/licenses/gpl-3.0.html)
![美术及其他资源许可证: All rights reserved](https://img.shields.io/badge/美术及其他资源-All%20rights%20reserved-orange.svg?style=flat)
![引擎许可证](https://img.shields.io/badge/引擎-Godot%20(MIT)-lightgrey)
> ⚠️ **重要** ⚠️  
> 本程序是**混合许可证**！  
> **代码**遵循 **GNU General Public License v3.0**，美术及其他资源为 **All Rights Reserved**（保留所有权利）。  
> 这意味着：您**可以**自由使用、修改、分发本项目的**代码**，但**必须**开源您的修改，且**不得**用于闭源商业项目。
> ⚠️ **IMPORTANT** ⚠️  
> This program is under a **mixed license**!  
> The **code** is licensed under the **GNU General Public License v3.0**, while the art and other resources are **All Rights Reserved**.
> 您可以在[项目 Wiki](https://github.com/minecraft-Yang458/RCQS-LSL/wiki) 中获得更多信息。
## 📖 关于项目
使用 **Godot 4.7.2** + **GDScript** 构建的Godot重构版作品。
- 常见问题解答：[FAQ](https://github.com/minecraft-Yang458/RCQS-LSL/wiki/FAQ)
## 🚀 快速开始
1. 下载 [Godot 4.7.2](https://godotengine.org/download/archive/4.7.2-stable/)
2. 导入 `project` 文件夹即可开始开发
> 如果需要编译游戏，还需要下载 [导出模板](https://downloads.godotengine.org/?version=4.7.2&flavor=stable&slug=export_templates.tpz&platform=templates)
>⚠️ 使用限制（重要）
> 
> - **代码部分**：遵循 **GPLv3** 协议。您可以在遵守 GPLv3 的前提下自由使用、修改、分发本项目的代码，但**修改后的代码也必须以 GPLv3 协议开源**。
> - **美术及其他资源**（图像、音频、剧本、字体等）：**保留所有权利**。未经版权人 **小硕道长（QQ: 1482883755）** 书面许可，**禁止**复制、分发、修改、改编、商业使用、移植、嵌入其他项目或制作衍生作品等任何形式的使用。
## 📜 版权信息
| 项目 | 版权归属 | 许可证 |
| :--- | :--- | :--- |
| 原作 | 小硕道长 | All Rights Reserved |
| Godot重构版开发 | Yang458 | — |
| 美术资源 | 小硕道长 | All Rights Reserved |
| 程序代码 | Yang458 | **GPLv3** |
| 游戏引擎 | Godot Engine | MIT License |
> ⚠️ **免责声明**：本游戏为Godot重构版作品，**非官方版本**。所有美术素材著作权归原作者所有，未经授权不得商用。
## 🗂️ 开发进度
- [x] UI 界面（半完成）
- [ ] 核心玩法
- [ ] 多平台测试
- [ ] 优化
- [ ] 发布
- [ ] 持续维护
## 🙏 致谢
- 感谢原作者 **小硕道长** 授权使用其原创美术及剧本素材，并支持本Godot重构版的开发。
- 感谢所有关注和支持本项目的玩家。
